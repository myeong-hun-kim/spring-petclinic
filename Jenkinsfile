pipeline {
  ...
  environment {
    ...
    AWS_CREDENTIALS = credentials('AWSCredential_NAME')
    ...
    stage('Docker Image Build') {
      steps {
        ...
        dir("${env.WORKSPACE}") {
          ...
          docker tag myeonghunkim/spring-petclinic:$BUILD_NUMBER s4616/spring-petclinic:latest
          """
        }
      }
    }
            ...
            steps {
                ...
                dir("${env.WORKSPACE}") {
                    ...
                    withAWS(region:"${REGION}", credentials:"${AWS_CREDENTIALS_NAME}"){
                      s3Upload(file:"deploy.zip", bucket:"user13-codedeploy-bucket")
                    } 
                    sh 'rm -rf ./deploy.zip'                 
                }        
            }
